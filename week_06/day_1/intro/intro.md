...
## STUPID - Bad Code Practices

Programmers love acronyms. They often describe a set of good or bad practices, like the DRY (Don't Repeat Yourself) principle, or KISS (Keep It Simple Stupid).

STUPID is a set of bad practices (or Dark Patterns), some of  which you most likely caught yourself using already. We won't spend much time talking about them, but here's a list of them, so you know what to look out for:

1. **Singleton** - having a one ever-present object that has a lot of responsibility (eg. DB accessor class)
2. **Tight coupling** - code being over-specific (eg. function returning an array of `student_name` strings, rather than an array of `Student` objects, from which we can take `student_name` strings)
3. **Untestability** - writing classes in a way that is very hard to test
4. **Premature Optimization** - spending time on improving speed slightly, when it's very unlikely it would become a real issue (and hence wasting valuable dev time)
5. **Indescriptive Naming** - calling methods, variables and clases meaningless names, or names that do not imply what the input and output is (eg. `Thing`, `FooBar`, `.play_game()`, `.do_it()`, `x`, `array`)
6. **Duplication** - repeating the same or very similar code in multiple places (eg. copy pasted methods varying only by a return type or a single operation) 

In response to these bad practices programmers developed a set of good practices described below.
