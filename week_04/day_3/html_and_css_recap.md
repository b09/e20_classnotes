...
> Show different forms and go through them to explain what they do. Remember the relationships between labels and the ```id``` attribute.

#### The difference between Id and Name attributes

- **id** is a unique identifier of an HTML element on your page
- **name** is a way you refer to your form elements once they were submitted to the server


Since historically many programmers (and web browsers) misunderstood this difference, you will see them used interchangeably on all sorts of old websites. To provide backwards compatibility, when your browser cannot find a reference to an `id` it will try to look for a `name`.

Simple rule is: Always use id. In form elements also use name, so that the server understands what you send over.

Look at the example below. Notice that label is referring to input's id, but id also had a name, that will be sent to the server. To indicate that the name belongs to the server, we can use the server-side case `name="your_name"` rather than html/css style `id="your-name"`.
...
<form>
	...
	<input type="text" id="your-name" name="your_name" placeholder="Enter your name" />
...
<form>
	<fieldset>
		...
		<input type="email" id="your-email" name="your_email" placeholder="Enter your email address" />		
	</fieldset>
	<fieldset>
		<legend>Password settings</legend>
		<label for="yourPassword">Set your password</label>
		<input type="password" name="your_password" id="your-password" />
	</fieldset>
</form>
```
...
<form>
	<fieldset>
	   ...
	   <input type="checkbox" id="fruit2" name="oranges" value="2" />   
		...
		<label for="fish-selection-1">I like tuna</label>
	   <input type="radio" id="fish-selection-1" name="fish_selection" value="tuna" />
	   	<label for="fish-selection-1">I like sardines</label>
	   <input type="radio" id="fish-selection-2" name="fish_selection" value="sardines" />   
	</fieldset>
</form>
```
...
<form>
	...
	<input type="file" id="your-file" name="your_file" />
</form>
```
